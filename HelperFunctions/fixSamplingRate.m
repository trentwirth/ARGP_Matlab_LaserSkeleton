function [ resampledTable ] = fixSamplingRate( pupTable, startTime,endTime,unixStartTime,prefRate,skipThese )
...
for vv = 1:length(VarNames)
    ...
    if isempty(strmatch(thisVarName, skipThese)) %resample data
        %tdw - fillmissing on "thisVar"
        thisVar = fillmissing(thisVar,'linear');
%         thisVar = fillmissing(thisVar,'movmedian',10); % try this later?
        ...
        if ~any(isnan(thisVar))
            ...
            if numFrames > length(timestamp)
                'This is a weird difference between eyes in this recording!!!! hopefully nothing breaks?!'
