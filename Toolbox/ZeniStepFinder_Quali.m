...
function [allSteps_HS_TO_StanceLeg] = ZeniStepFinder_Quali(w)

quali_fr_mar_dim = w.markers_fr_mar_dim;
qualiMarkerNames = w.qualiMarkerNames;
fps = mean(w.fps);
walks = w.trials;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Y IS UP in qualisys data :: TDW 2021-11-11
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
zeniMarkerData = quali_fr_mar_dim;
...
rFootID = find(strcmp('RAnkleIn', qualiMarkerNames));
rAnkX_noZ = squeeze(zeniMarkerData(:,rFootID,1)); % pull out rFootID marker as X vector
rAnkY_noZ = squeeze(zeniMarkerData(:,rFootID,2)); % pull out rFootID marker as Y vector
rAnkZ_noZ = squeeze(zeniMarkerData(:,rFootID,3)); % pull out rFootID marker as Z vector

lFootID = find(strcmp('LAnkleIn', qualiMarkerNames));
lAnkX_noZ = squeeze(zeniMarkerData(:,lFootID,1)); % pull out lFootID marker as X vector
lAnkY_noZ = squeeze(zeniMarkerData(:,lFootID,2)); % pull out lFootID marker as Y vector
lAnkZ_noZ = squeeze(zeniMarkerData(:,lFootID,3)); % pull out lFootID marker as Z vector

%define root as 'Body'
% BodyID = find(strcmp('Hips',qualiMarkerNames)); %find body marker
% root = squeeze(zeniMarkerData(:,BodyID,:));

% Because there isn't a "hip" marker, feed in the mean hips that I
% calculated earlier in the script. Might want to calculate ankle center,
% too
...
for f = 1:numel(zeniMarkerData(:,1,1)) %f = Frame
    for m = 1:numel(zeniMarkerData(1,:,1)) %m = Marker
        zeniMarkerData(f,m,1) = zeniMarkerData(f,m,1) - root(f,1);
        zeniMarkerData(f,m,2) = zeniMarkerData(f,m,2) - root(f,2);
        zeniMarkerData(f,m,3) = zeniMarkerData(f,m,3) - root(f,3);
    end
end


rThighXY = squeeze(zeniMarkerData(:,strcmp('RThigh', qualiMarkerNames), [1 2])); % pull out lFootID marker as X vector

[rThighTheta, rThighRho] = cart2pol(rThighXY(:,1), rThighXY(:,2));

for f = 1:numel(zeniMarkerData(:,1,1)) %f = Frame
    for m = 1:numel(zeniMarkerData(1,:,1)) %m = Marker
        
        X = zeniMarkerData(f,m,1);
        Y = zeniMarkerData(f,m,2);
        ...
        x_r = ...
            ...
            Y * sin(rThighTheta(f));    %y*sin(theta)
        
        
        y_r = ...
            -X * sin(rThighTheta(f))+... %x*cos(theta)
            Y * cos(rThighTheta(f));    %y*sin(theta)
        
        
        zeniMarkerData(f,m,1) = x_r;
        zeniMarkerData(f,m,3) = y_r;
        
    end
end


rThighXYrot = squeeze(zeniMarkerData(:,strcmp('RightThigh', qualiMarkerNames), [1 2])); % pull out lFootID marker as X vector




rAnkX = squeeze(zeniMarkerData(:,rFootID,1)); % pull out rFootID marker as X vector
rAnkY = squeeze(zeniMarkerData(:,rFootID,2)); % pull out rFootID marker as Y vector
rAnkZ = squeeze(zeniMarkerData(:,rFootID,3)); % pull out rFootID marker as Z vector

lAnkX = squeeze(zeniMarkerData(:,lFootID,1)); % pull out lFootID marker as X vector
lAnkY = squeeze(zeniMarkerData(:,lFootID,2)); % pull out lFootID marker as Y vector
lAnkZ = squeeze(zeniMarkerData(:,lFootID,3)); % pull out lFootID marker as Z vector
...
walkDir = 1; % this is hard coded for now because I'm working with one trial TDW 2021-11-11
...
rAnkVelY = [0; diff(rAnkY)];
rAnkVel =  rAnkVelY*mean(fps)/1000; %now that I'm rotating the data, all the action should be happening in the Z direction. <<-Hey, FutureJon. Nice to see you. The problem is probably here <<--
% rAnkVel = plus(rAnkVelX, rAnkVelZ)*mean(avg_fps)/1000; <<-- this is what it used to look like ^^
rAnkVelRaw = rAnkVel.* walkDir;
rAnkVel = butterLowZero(order, cutoff, mean(fps), rAnkVel);
...
lAnkVelY = [0; diff(lAnkY)];
lAnkVel = lAnkVelY*mean(fps)/1000;
lAnkVelRaw = lAnkVel.* walkDir;
lAnkVel = butterLowZero(order, cutoff, mean(fps), lAnkVel);
...
lAnkVel_noZ = plus(lAnkVelX_noZ, lAnkVelZ_noZ)*mean(fps)/1000;
...
% TDW 2021\11\12 - got rid of this step because we can trust qualisys data
% better than we can trust the shadow data Jon was using.

% %remove Same-Leg Gait events that are below a threshold
% minEventDur = round(.200/(1/fps)); %200ms (0.2sec) = 20frames @~100fps
% 
