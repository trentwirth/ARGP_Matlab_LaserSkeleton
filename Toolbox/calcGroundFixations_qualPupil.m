function [ gazeGroundIntersection ] = calcGroundFixations_qualPupil( calibPoint, eyeballCenterXYZ, gazeXYZ )
...
%5mm underneath the calib point)
...
for ii = frames
    if min(size(eyeballCenterXYZ)) == 1 %if only only one frame is input (this if/else statement probably isn't necessary, but whatevs)
        ...
        grHeight = calibPoint(3); % replaced with the z of the calibPoint
    ...
    V0 = [0 0 grHeight]; % V) - any point on the ground plane
    n = [0 0 grHeight+10]; %normal Vector of the ground plane (I think?)
    ...
    if debugPlot
        % % %
        % % % debug plot   Trent's question: what does this debug plot do?
        % % %
        figure(3)
        cla
