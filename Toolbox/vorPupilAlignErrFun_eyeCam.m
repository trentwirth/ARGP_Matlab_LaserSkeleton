function [camAlignErr] = vorPupilAlignErrFun_eyeCam(vData, camAlignGuess_xyz_a)
...
camRotMatGuess = eul2rotm(camAlignGuess_xyz_a, 'XYZ');
...
qual_dim_mar_fr = vData.qual_dim_mar_fr; 
% shadow_fr_mar_dim = vData.shadow_fr_mar_dim;

% the first data point just so happens to be hips, so it's a good proxy of
% COM, intentional? coincidence across qualisys and shadow?
comXYZ = squeeze(qual_dim_mar_fr(:,1,:));
...
confThresh = 0;% was .8, bumped it up to .9
